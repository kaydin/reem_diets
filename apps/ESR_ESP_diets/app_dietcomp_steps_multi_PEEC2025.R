...
predlist <- read.clean.csv("apps/ESR_ESP_diets/predlist_master.csv")
...
for (this.model in c("EBS","NBS","AI","WGOA","EGOA")){
  samples <- predprey_tables_all(model=this.model)
  pred_tot_combined <- rbind(pred_tot_combined, samples)
  sample_tots <- samples %>%
    group_by(model,pred_name,year) %>%
    summarize(samples=n(),.groups="keep")
  sample_list[[this.model]] <- data.frame(sample_tots %>% filter(year==this.year))
}

pred_tot_sums <- pred_tot_combined %>%
  group_by(model,pred_name,year) %>%
  summarize(samples=n(),.groups="keep")

#predlist_year <- unique(pred_tot_sums$pred_name[pred_tot_sums$year==this.year])
#pred_year_sums <- pred_tot_sums %>% filter(pred_name %in% predlist_year)

# renamed this file to predlist_master.csv in directory
#output.csv(predlist_year,"predlist_master")

#library(formattable)
#formattable(pred_tot_combined)
...
#this.model <- "EBS"
  ...
  preds      <- predlist %>% 
    left_join(sample_list[[this.model]],by=c("base_name"="pred_name")) %>% 
    filter(!is.na(samples)) 
    
    #predlist #%>% filter(model==this.model)
  ...
  for (p in pred_names){
    ...
    lw <- get_lw(p, this.model)
    pred_params[[p]]$lw_b   <- lw$lw_b #pdat$b_l_mm_g
    pred_params[[p]]$lw_a   <- lw$lw_a #pdat$a_l_mm_g*(10^pdat$b_l_mm_g)
    pred_params[[p]]$lw_a_mm<- lw$lw_a/(10^lw$lw_b) #pdat$a_l_mm_g
...
diet_cons_domain_combined <- diet_cons_domain_combined %>%
  mutate(biomass_sciprop_tons=tot_wlcpue_tons * dietprop_sci)
...
pl_dat <- preylengths %>% 
  ...
  rename(survey_id = survey_definition_id) %>%
  filter(survey_id==SURVEY_IDS["EBS"]) %>%
  filter(pred_nodc==8791030401 & prey_nodc %in% c("6187010302")  )
  #filter(pred_nodc==8791030401 & prey_nodc %in% c("6187010300", "6187010301")  )
...
png("Crab_size_tanner.png",1600,2400)
ldat <- len_dat
ggplot(ldat, aes(x = plens, y = as.factor(years), fill="#5072B2")) +
  geom_density_ridges() +
  theme_ridges() + 
  xlab("Crab carapace width (mm)") +
  ylab("") +
  labs(title="Tanner crab size in EBS Pacific cod stomachs")+
  xlim(0,80) +
  theme(legend.position = "none",
        axis.title.x = element_text(hjust = 0.5,size=48,face="bold"),
        plot.title = element_text(hjust = 0.5,size=64,face="bold"),
...
png("Crab_size_recent_tanner.png",1600,2400)
ldat <- len_dat[len_dat$years>2004,]
ggplot(ldat, aes(x = plens, y = as.factor(years), fill="#5072B2")) +
  geom_density_ridges() +
  theme_ridges() + 
