...
get_dietprop_biomassweighted_bylength <- function(diet_tables = list(
  ppfile   = ppfile  ,
...
# Get diet proportions (g prey/g pred (of length bin) / strata) table (by length bin) by running "get_dietcomp_bylength_function.r" function
dietprop.strata <- get_dietcomp_bylength(predators = predators)
...
#rename PRED/species names to match in diet and biomass database
...
#join new pred biomass by length table with diet proportion table  
...
dietprop.strata3<-dietprop.strata2 %>%
  ...
  mutate (PredBiom.JuvAdult.Str=sum(new.stratum_biomass.Lbin)) %>%   #prey biomass (Juv/Adult) per stratum
  mutate (PreyBiom.JuvAdult.Str=sum(preyBiom.Lbin.Str))   #prey (mt) consumed by pred (Juv/Adult) per stratum

return(dietprop.strata3)
}  #end function


#test function is working
x <- get_dietprop_biomassweighted_bylength(predators = c("W.pollock")) #P.cod, Arrowtooth
head(x)
