...
#for (PRED in predators){
  for (i in length(NODC)){
  # Pick the predator 
  #ThisPred <- preds[[PRED]] 
  NODC.code=NODC[i]
  preddat <- rawdat[rawdat$PRED_NODC %in% NODC.code,]  
  LW.intercept_a <- FunGrp2$intercept_a[which(FunGrp2$NODC==NODC.code)]
  LW.slope_b <- FunGrp2$slope_b[which(FunGrp2$NODC==NODC.code)]
  ...
  #for (STRAT in names(stratblock)){    #for NPFMC areas (superstrat)
   ...
   #   lencodes <- LL 
      ...
      #  yearcodes <- YY
        ...
        SELPRED <- SURV[which(SURV$STRATUM==stratcodes),]  #& SURV$YEAR %in% yearcodes   ,] 
...
x <-get_dietcomp_NoLength(predators = 8857040901) #NODC code for YFS
head(x)
