...
get_cpue <- function(racebase_tables = list(
  ...
  #KYA changed == to %in% to allow multiple codes to be passed
  sp_catch <- catch %>%
  ...
  x <- dat %>%
    ...
    # KYA removed replace_na to allow multiple species codes
  ...
  # KYA added to support multiple codes
  if (length(speciescode) == 1){
    x <- x %>% replace_na(list(species_code = speciescode))
  }
  return(x)
}
